...
public class LetterSquare {
    ...
    private boolean isValid(String letter, int wordNum, int charNum) {        
        if (wordNum == 0 && charNum == 0) {
            ...
            return lastLetter(this.words[wordNum-1]).equals(letter);
        }

        else if (onSameSide(letter, lastLetter(this.words[wordNum]))) {
            return false;
        }
        ...
        return alreadyUsed(newWord) == false && dictionary.hasString(newWord);
    }
    ...
    private boolean solveRB(int wordNum, int charNum, int maxWords) {
        String newWord = this.words[wordNum];
        if (allLettersUsed() && dictionary.hasFullWord(newWord) && newWord.length() >= 3) {
        ...
        for (int i = 0; i < letters.length; i++) {
            if (isValid(letters[i], wordNum, charNum)) {
                addLetter(letters[i], wordNum);
                newWord = this.words[wordNum];

                    if (solveRB(wordNum, (charNum + 1), maxWords)) {
                        return true;
                    }

                    else if (dictionary.hasFullWord(newWord) && newWord.length() >= 3) {
